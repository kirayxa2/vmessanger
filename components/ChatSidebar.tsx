...
export default function ChatSidebar({
  ...
  // Обертка-контейнер для создания эффекта капсулы
  const CapsuleWrapper = ({ children }: { children: React.ReactNode }) => (
    <div className="w-full h-full flex flex-col relative bg-[#0f1721] p-2 pb-0">
      ...
      {/* Отступ снизу для визуальной стыковки с док-баром */}
      <div className="h-[2px]" /> 
    </div>
  )
            ...
            <div className="flex flex-col">
              {filteredConversations.map((chat, index) => {
                ...
                const avatarContent = isSaved ? <Bookmark size={22} className="text-white" />
                  : isSystem ? <img src="/logo (1).ico" alt="Vortex" className="w-full h-full object-cover" />
                ...
                return (
                  <motion.div key={chat.id} onClick={() => onSelect?.(chat.id.toString())}
                    ...
                    className={`p-[9px] px-[12px] mb-[2px] mx-2 rounded-[16px] cursor-pointer flex items-center gap-[12px] transition-all ${
                      isSelected ? "text-white shadow-lg scale-[1.01]" : "bg-transparent hover:bg-white/5"
                    }`}
                    style={isSelected ? { backgroundColor: ACCENT } : {}}>
                    <div className="w-[52px] h-[52px] rounded-full flex items-center justify-center shrink-0 overflow-hidden text-white shadow-md"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-[1px]">
                        <div className={`text-[14px] truncate pr-2 ${isSystem ? "text-gray-500" : (isSelected ? "text-white/90" : "text-gray-400")}`}>
